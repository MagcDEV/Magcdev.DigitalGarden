---
title: Manuel Guevara CV
draft: false
tags:
---
# Manuel Guevara

Full-Stack Developer ASP.NET Core | C# | JAVA | Angular | ReactJS
Buenos Aires, Buenos Aires Province, Argentina 📍

---

## 📞 Contact

* **Mobile:** 1122747644 📱
* **Email:** manuel_guevarac@outlook.com 📧
* **LinkedIn:** [linkedin.com/in/manuelaguevarac](https://www.linkedin.com/in/manuelaguevarac) 🔗
* **CV in PDF:** [Manuel Guevara CV](https://drive.google.com/file/d/1wJFFjjE44yQtZWas8ftTsdfz_kIIuUkB/view?usp=sharing) 📄

---

## 📝 Summary

Results-oriented Full-Stack Developer specializing in scalable, high-performance software solutions using Microsoft technologies (.NET, C#, Azure). Proven expertise is complemented by strong frontend development skills in React and Angular. Highly adaptable, with a solid foundation in software principles enabling rapid learning and productivity with new tools and frameworks. Possesses a keen eye for detail, ensuring application quality through thorough testing, debugging, and code refactoring for optimal performance and maintainability. Experienced in deploying fully functional applications that drive business growth. Eager to leverage comprehensive technical skills to deliver exceptional results.

---

## ✨ Top Skills

* Azure DevOps Services
* .NET Architecture
* Microsoft Azure ☁️
* APIs and Microservices
* C#
* ASP.NET Core
* ReactJS
* Angular
* Node.js
* SQL Server 💾
* Java
* Event-Driven Architecture

---

## 👨‍💻 Experience

**Globant**
*Software Engineer*
*May 2024 - Present*
*Buenos Aires Province, Argentina*
* Developed a decoupling application using .NET and C#, implementing an event-driven architecture (producers/consumers) to isolate a legacy database, enabling secure access for third-party applications.
* Currently contributing to a large-scale engagement management application utilized by Deloitte consultants.

**Accenture**
*Senior Dotnet Developer*
*December 2023 - May 2024 (6 months)*
*Buenos Aires Province, Argentina*
* Worked on a home banking application for Banco Galicia, developing and maintaining microservices using .NET and C#.

**Infosoft CA**
*Programador full stack*
*April 2023 - October 2023 (7 months)*
*Venezuela*

**WebCreek**
*Software Engineer*
*January 2022 - January 2023 (1 year 1 month)*
*Venezuela*
* Worked in deployments and app maintenance 100% remote with a diverse team of developers.
* Establishment of procedures for fast deployment for testing and defect solving.
* Writing documentation for the deployment process.

**Wadara Servicios Tecnológicos**
*Developer Front-End: ReactJs / NodeJs / SQL Server*
*August 2021 - January 2022 (6 months)*
*Caracas, Federal District, Venezuela*
* Desarrollo de aplicaciones web (Web application development)

**Banco de Venezuela**
*Software Developer*
*July 2017 - August 2021 (4 years 2 months)*
*Caracas Area, Venezuela*
* Implementation of databases with customer data for credit evaluation purposes.
* Development of credit simulators connected to credit behavior databases.

---

## 🎓 Education

**Universidad Central de Venezuela**
*Economista, Economics*
*(2007 - 2021)*

---

## 🌐 Languages

* Español (Native or Bilingual Proficiency)
* English

---

## 📜 Certifications

* [INGLES COMO LENGUA EXTRANGERA](https://drive.google.com/file/d/166bgxLeqQfQ_3pN_Rk9aGfVc0OZb0nJA/view?usp=sharing)
* [EF SET Certificate](https://cert.efset.org/3uvJNp)
* Getting and Cleaning Data
* Responsive Web Design
* APIs and Microservices